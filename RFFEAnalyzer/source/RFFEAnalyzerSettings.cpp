...
RFFEAnalyzerSettings::RFFEAnalyzerSettings() : mSclkChannel(UNDEFINED_CHANNEL), mSdataChannel(UNDEFINED_CHANNEL) {
  ...
  mShowParityInReportInterface->SetTitleAndTooltip("Show Parity in Report?", "Check if you want parity information in the exported file");
  AddInterface(mShowParityInReportInterface.get());

  mShowBusParkInReportInterface.reset(new AnalyzerSettingInterfaceBool());
  mShowBusParkInReportInterface->SetTitleAndTooltip("Show BusPark in Report?", "Check if you want bus park information in the exported file");
